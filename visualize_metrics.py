...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot train/validation metrics from latest Lightning training run."
    )
    parser.add_argument(
        "--log-root",
        type=Path,
        default=Path("logs/segrnn_experiment"),
        help="Root directory containing version_* run folders.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("training_validation_metrics.png"),
...
def get_metric_series(
    ...
    # Fallback: treat logging step as epoch-like index.
...
def main() -> None:
    ...
    unique_tags = sorted(df["tag"].astype(str).map(normalize_tag).unique()) if not df.empty else []
    ...
    print(f"Detected metric tags: {unique_tags if unique_tags else 'None'}")
